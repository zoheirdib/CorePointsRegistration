...
namespace ZDEngine
{
	...
	class ZDCPDAffineRegistration
	{
		/*!
			...
			WHAT THIS OBJECT REPRESENTS
				this object implements the affine registration algorithm as discribed if the following paper
	...
	public:
		...
		// to use this object with matrices of the same number of columns (DIM).
		...
		typedef ZDPointTransformAffine<type, DIM, mem_manager_type, layout_type > affinePointTransform_type;
		...
		struct AffineResult
		{
			...
			affinePointTransform_type transform;
			double sigma2;
		};
		...
		!*/
		void getTransform(affinePointTransform_type& transform)
		{
			transform = result.transform;
		}
		...
		void compute_one(const target_matrix_type& fixed, const source_matrix_type& moving,
			...
			double sigma2, AffineResult& result)
		{
			...
			matrix<type> b1 = trans(probabilities.px) * moving - np * mu_x * trans(mu_y);
			
			matrix<type> b2 =
				trans(pointwise_multiply(moving, repmat(probabilities.p1, 1, moving.nc())))
				*
				moving -
				np * mu_y * trans(mu_y);

			result.transform.rotation = b1 * inv(b2);
			result.transform.translation = mu_x - result.transform.rotation * mu_y;
			result.sigma2 =
				std::abs(sum(pointwise_multiply(pow(fixed, 2),
					repmat(probabilities.pt1, 1, cols)))
					-
					np * trans(mu_x) * mu_x -
					trace(b1 * trans(result.transform.rotation))) /
					(np * cols);
			result.points = moving * trans(result.transform.rotation) +
