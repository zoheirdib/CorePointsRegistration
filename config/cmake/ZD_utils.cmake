...
MACRO(SOURCE_GROUP_BY_FOLDER target)
  ...
  FOREACH(file ${${target}_SRC} ${${target}_HEADERS})
    file(RELATIVE_PATH relative_file "${ZD_ROOT_SOURCE_DIR}/${target}/" ${ZD_ROOT_SOURCE_DIR}/${target}/${file})
	 #message(WARNING "File not found: ${ZD_ROOT_SOURCE_DIR}/${target}/${file}")
	 #message(WARNING "RELATIVE_PATH: ${relative_file}")
...
#
macro(zd_configure_file infile outfile installprefix)
...
# A macro to setup configuration and installation of header files
#
macro(zd_configure_file_copyonly infile outfile installprefix)
...
# ZDEngine for adding a library, setting it's properties, and
...
#                      ZD_INSTALL_INCLUDE_DIR if ZD_INSTALL_INCLUDE_DIR is
#                      not its default value; otherwise, the relative path in
#                      the ZDEngine source tree is used.
#
macro( zd_add_library )
  unset(lib_srcs)
  ...
  if( ${num_src_files} GREATER 0 )
    ...
    set_property(GLOBAL APPEND PROPERTY ZDTargets_MODULES ${lib_name})
    if(ZD_LIBRARY_PROPERTIES)
       set_target_properties(${lib_name} PROPERTIES ${ZD_LIBRARY_PROPERTIES})
    endif()

    # Installation
    if(NOT ZD_INSTALL_NO_LIBRARIES)
      install(TARGETS ${lib_name}
        EXPORT ${ZD_INSTALL_EXPORT_NAME}
        RUNTIME DESTINATION ${ZD_INSTALL_RUNTIME_DIR} COMPONENT RuntimeLibraries
        LIBRARY DESTINATION ${ZD_INSTALL_LIBRARY_DIR} COMPONENT RuntimeLibraries
        ARCHIVE DESTINATION ${ZD_INSTALL_ARCHIVE_DIR} COMPONENT Development)
    endif()
  endif()
  if(NOT ZD_INSTALL_NO_DEVELOPMENT)
    # If ZD_INSTALL_INCLUDE_DIR is the default value
    if("${ZD_INSTALL_INCLUDE_DIR}" STREQUAL "include/ZDEngine")
      ## Identify the relative path for installing the header files and txx files
      string(REPLACE ${ZD_ROOT_SOURCE_DIR} "${ZD_INSTALL_INCLUDE_DIR}" relative_install_path ${CMAKE_CURRENT_SOURCE_DIR})
      ...
      set(relative_install_path "${ZD_INSTALL_INCLUDE_DIR}")
      ...
      if(${CMAKE_VERSION} VERSION_GREATER 2.8.11.2)
        target_include_directories(${lib_name}
          PUBLIC
            $<BUILD_INTERFACE:${CMAKE_CURRENT_SOURCE_DIR}>
            $<INSTALL_INTERFACE:${ZD_INSTALL_INCLUDE_DIR}>
        )
      endif()
    endif()
...
#  )
#
macro(SET_ZD_LIBRARY_PROPERTIES)
  set(options USE_HIDDEN_VISIBILITY)
  set(oneValueArgs TARGET_NAME BASE_NAME INSTALL_DIR EXPORT_HEADER_FILE FOLDER)
  ...
  zd_generate_export_header(${LSLHVP_TARGET_NAME}
